...
k1p, k2p = 0, 2
ap, ep = 1.0, 0.1
Ω1p, Ω2p = frequencies_from_ae(ap,ep,model,Orbitalparams)

ωres = k1*Ω1 + k2*Ω2

SecularResponse.CCPrepare!(a,e,Ω1,Ω2,k1,k2,lharmonic,0.0+0.0*im,model,couplingLM,Linearparams)
ccLM = SecularResponse.CouplingCoefficient(ap,ep,Ω1p,Ω2p,k1p,k2p,lharmonic,model,couplingLM,Linearparams)

SecularResponse.CCPrepare!(a,e,Ω1,Ω2,k1,k2,lharmonic,0.0+0.0*im,model,couplingLB,Linearparams)
ccLB = SecularResponse.CouplingCoefficient(ap,ep,Ω1p,Ω2p,k1p,k2p,lharmonic,model,couplingLB,Linearparams)
...
_, _, dFdtLM = SecularResponse.GetSecularResContrib(J,L,k1,k2,k1p,k2p,lharmonic,model,DF,ndFdJ,couplingLM,Secularparams)
_, _, dFdtLB = SecularResponse.GetSecularResContrib(J,L,k1,k2,k1p,k2p,lharmonic,model,DF,ndFdJ,couplingLB,Secularparams)
_, _, dFdtBL = SecularResponse.GetSecularResContrib(J,L,k1,k2,k1p,k2p,lharmonic,model,DF,ndFdJ,couplingBL,Secularparams)
